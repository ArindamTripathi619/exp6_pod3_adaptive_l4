...
class ExecutionTrace(BaseModel):
    ...
    # New fields for audit requirements
    propagation_path: List[Dict[str, Any]] = Field(
        default_factory=list,
        description="Layer-by-layer attack propagation tracking with decisions and risk scores"
    )
    
    bypass_mechanisms: List[str] = Field(
        default_factory=list,
        description="Detected mechanisms used to bypass defenses (e.g., obfuscation, role-play)"
    )
    
    trust_boundary_violations: List[Dict[str, Any]] = Field(
        default_factory=list,
        description="Detected violations of trust boundaries (privilege escalation, context contamination)"
    )
    
    coordination_enabled: bool = Field(
        default=False,
        description="Whether inter-layer coordination was enabled for this request"
    )
